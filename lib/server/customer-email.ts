...
export async function sendBookingConfirmation(input: { to: string; firstName: string; reference: string; service: string; date: string; time: string; portalUrl: string; clientId?: string; profileId?: string; bookingId?: string }) {
  try {
    ...
    return sendTransactionalEmail({
      templateKey: "booking-request-received", logicalIdempotencyKey: `booking-confirmation-${input.bookingId ?? input.reference}`,
      triggerKey: "booking.created", clientId: input.clientId, recipientProfileId: input.profileId, bookingId: input.bookingId,
      recipientName: input.firstName, recipientSnapshot: { firstName: input.firstName, email: input.to },
      renderContext: { reference: input.reference, service: input.service, date: input.date, time: input.time, portalUrl: input.portalUrl },
      message: {
      to: input.to,
      from,
      replyTo,
