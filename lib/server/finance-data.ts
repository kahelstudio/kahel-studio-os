...
export async function getFinanceKpis(): Promise<FinanceKpis> {
  ...
  const posTotal = (posSales ?? []).reduce((s: number, p: any) => s + (Number(p.total) ?? 0), 0);

  const unreconciled = bookletRemaining;
...
export async function getExpenses(): Promise<ExpenseRow[]> {
  ...
  const maintenanceExpenses: ExpenseRow[] = (maintenance ?? []).map((m: any) => ({
    id: m.id,
    ref: m.asset_label,
  ...
  const payrollExpenses: ExpenseRow[] = (payroll ?? []).map((p: any) => ({
    id: p.id,
    ref: p.reference,
    category: "Payroll",
    type: "Salary",
