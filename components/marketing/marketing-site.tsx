...
function Photo({ alt, position = "50% 35%", preload = false, sizes = "(min-width: 1081px) 33vw, (min-width: 641px) 50vw, 100vw" }: { alt: string; position?: string; preload?: boolean; sizes?: string }) {
  return <Image src="/Solo_Liza Burzon Bino_9A.jpg" alt={alt} fill sizes={sizes} preload={preload} style={{ objectFit: "cover", objectPosition: position, backgroundColor: "#2b2927" }} />;
}
...
function Home({ go }: { go: (page: Page, category?: ServiceCategory) => void }) {
  ...
  return <>
    ...
    <section className={`${styles.section} ${styles.softSection}`}><div className={styles.container}><div className={styles.centerHead}><Eyebrow>What we shoot</Eyebrow><h2>Two ways to work with us.</h2></div><div className={styles.serviceTiles}>{[{ title: "In the studio", kicker: "Studio sessions", description: "Portraits, branding, group and mini shoots, from ₱999.", position: "30% 48%", category: "sessions" as const }, { title: "On your day", kicker: "Events", description: "Debut, christening, birthdays and celebrations, from ₱5,000.", position: "72% 43%", category: "events" as const }].map((item) => <button type="button" key={item.title} onClick={() => go("services", item.category)}><Photo alt={item.title} position={item.position} /><span className={styles.tileScrim} /><span className={styles.tileCopy}><Eyebrow warm>{item.kicker}</Eyebrow><strong>{item.title}</strong><small>{item.description}</small><b>View rates <ArrowRight size={15} /></b></span></button>)}</div></div></section>
