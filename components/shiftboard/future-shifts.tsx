...
export function FutureShifts({ mode }: { mode: "week" | "month" }) {
  ...
  const days = Array.from({ length: 7 }, (_, index) => { const date = new Date(starts[0]); date.setDate(date.getDate() + index); return date; });
  return <div className="mt-[22px] grid gap-3.5 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7">{days.map((day) => { const dayOfWeek = day.getDay(); const rows = shifts.filter((shift) => shift.dayOfWeek === dayOfWeek); return <section key={day.toISOString()} className="min-h-[200px] rounded-card border border-[var(--color-border)] bg-[var(--color-canvas)] p-3"><div className="mb-3 flex items-baseline gap-1.5 px-2 py-1"><span className="font-display text-sm font-semibold">{day.toLocaleDateString("en-PH", { weekday: "short", timeZone: "Asia/Manila" })}</span><span className="text-xs text-[var(--color-text-muted)]">{day.getDate()}</span></div>{rows.length ? <ShiftRows rows={rows} /> : <p className="px-2 py-6 text-center text-xs text-[var(--color-text-muted)]">No shifts yet</p>}</section>; })}</div>;
}
