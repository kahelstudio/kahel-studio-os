...
export function NotificationMenu() {
  ...
  useEffect(() => {
    let active = true;
    fetch("/api/notifications", { cache: "no-store" }).then(async (response) => response.ok ? await response.json() as { notifications?: Notification[] } : null).then((body) => { if (active && body?.notifications) setNotifications(body.notifications); }).catch(() => undefined);
    return () => { active = false; };
  }, []);
