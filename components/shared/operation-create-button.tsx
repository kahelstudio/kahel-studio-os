...
const configs: Record<OperationKind, { title: string; description: string; submit: string; adminOnly?: boolean; permission?: string; fields: Field[] }> = {
  ...
  equipment: { title: "Add equipment", description: "Register a serialized studio asset.", submit: "Add equipment", fields: [{ name: "id_tag", label: "ID tag", required: true, placeholder: "KAHL-001" }, { name: "serial", label: "Equipment serial", required: true, placeholder: "AK8654564564654" }, { name: "name", label: "Equipment name", required: true }, { name: "category", label: "Category", required: true }, { name: "status", label: "Status", type: "select", options: ["available", "out", "maint"], required: true }, { name: "location", label: "Location" }, { name: "note", label: "Note", type: "textarea" }] },
