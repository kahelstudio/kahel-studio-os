...
const configs: Record<OperationKind, { title: string; description: string; submit: string; adminOnly?: boolean; permission?: string; fields: Field[] }> = {
  booking: { title: "New booking", description: "Create a studio booking. Existing clients are matched by phone number.", submit: "Create booking", permission: "bookings.manage", fields: [
    ...
    { name: "bookingSource", label: "Booking source", type: "select", options: ["staff", "walk_in", "phone", "messenger", "instagram"], required: true },
    { name: "promoCode", label: "Promo code", placeholder: "Optional" },
    { name: "internalNotes", label: "Internal notes", type: "textarea", placeholder: "Staff-only notes (not visible to client)" },
  ] },
  ...
  equipment: { title: "Add equipment", description: "Register a studio asset. Its KAHL ID tag is assigned automatically.", submit: "Add equipment", fields: [{ name: "serial", label: "Equipment serial (optional)", placeholder: "NA if unavailable" }, { name: "name", label: "Equipment name", required: true }, { name: "category", label: "Category", required: true }, { name: "status", label: "Status", type: "select", options: ["available", "out", "maint"], required: true }, { name: "location", label: "Location" }, { name: "note", label: "Note", type: "textarea" }] },
