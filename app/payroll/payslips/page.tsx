    ...
    <div className="max-w-[1100px] p-10 pb-14 pt-8">
      ...
      <div className="mt-6 overflow-hidden rounded-card border border-[var(--color-border)] bg-[var(--color-surface)]">
        ...
        {PAYROLL_PAYSLIPS.map((p) => (
          <div
            key={p.ref}
            className="grid h-14 grid-cols-[1.4fr_1.6fr_1.2fr_1fr_1fr] items-center border-b border-[var(--color-ink-100)] px-5 text-sm last:border-b-0 hover:bg-[var(--color-canvas)]"
          >
            <div className="font-mono text-xs text-[var(--color-text-muted)]">{p.ref}</div>
            <div className="font-semibold">{p.emp}</div>
            <div className="text-[var(--color-ink-700)]">{p.period}</div>
