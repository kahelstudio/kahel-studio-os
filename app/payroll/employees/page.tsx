    ...
    <div className="max-w-[1200px] p-10 pb-14 pt-8">
      ...
      <div className="mt-6 overflow-hidden rounded-card border border-[var(--color-border)] bg-[var(--color-surface)]">
        ...
        {PAYROLL_EMPLOYEES.map((e) => (
          <Link
            key={e.id}
            href={e.id === "EMP-002" ? "/payroll/employees/emp-002" : "#"}
            className="grid h-14 grid-cols-[1.8fr_1fr_1fr_1fr_1.2fr_1fr] items-center border-b border-[var(--color-ink-100)] px-5 text-sm last:border-b-0 hover:bg-[var(--color-canvas)]"
          >
            ...
            <div className="text-[var(--color-ink-700)]">{e.basis}</div>
            <div className="text-[var(--color-ink-700)]">{e.rate}</div>
            <div className="font-mono text-xs text-[var(--color-text-secondary)]">{e.method}</div>
            <div>
              <span
