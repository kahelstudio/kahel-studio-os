    ...
    <div className="max-w-[1200px] p-10 pb-14 pt-8">
      ...
      <div className="mt-6 overflow-hidden rounded-card border border-[var(--color-border)] bg-[var(--color-surface)]">
        ...
        {PAYROLL_ADJUSTMENTS.map((a) => (
          <div
            key={a.ref}
            className="grid h-14 grid-cols-[1fr_1.6fr_1.4fr_1fr_1fr_1.2fr] items-center border-b border-[var(--color-ink-100)] px-5 text-sm last:border-b-0 hover:bg-[var(--color-canvas)]"
          >
            <div className="font-mono text-xs text-[var(--color-text-muted)]">{a.ref}</div>
            <div className="font-semibold">{a.emp}</div>
            <div className="text-[var(--color-ink-700)]">{a.kind}</div>
            <div className="text-right font-display font-semibold" style={{ color: a.dirColor }}>
              {a.amt}
            </div>
            <div className="font-mono text-xs text-[var(--color-text-secondary)]">{a.run}</div>
            <div>
              <span
