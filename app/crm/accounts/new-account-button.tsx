...
export function NewAccountButton() {
  ...
  return <><button type="button" onClick={() => setOpen(true)} className="flex min-h-11 items-center gap-1.5 rounded-control bg-[var(--color-kahel-500)] px-4 font-display text-sm font-semibold text-white hover:bg-[var(--color-kahel-600)]"><Plus className="h-4 w-4" /> New customer</button>{open && <NewAccountDialog onClose={() => setOpen(false)} onCreated={(id) => { setOpen(false); fireToast("Account created.", "success"); router.push(`/crm/accounts/${id}`); router.refresh(); }} />}</>;
}
