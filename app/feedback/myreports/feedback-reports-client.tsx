...
import type { FeedbackStatus } from "@/lib/sample-data";
import { cn } from "@/lib/utils";
import type { FeedbackReport as ServerFeedbackReport } from "@/lib/server/feedback-data";
...
function mapReports(server: ServerFeedbackReport[]): FeedbackReport[] {
  return server.map((r) => ({
    iid: r.iid,
    title: r.title,
    summary: r.summary ?? "",
...
export function FeedbackReportsClient({ reports: serverReports }: { reports: ServerFeedbackReport[] }) {
  ...
  const reports = useMemo(() => mapReports(serverReports), [serverReports]);
...
function ReportRow({ report: r }: { report: FeedbackReport }) {
  const kind = FEEDBACK_KIND_META[r.kind as keyof typeof FEEDBACK_KIND_META] ?? { bg: "var(--color-surface-muted)", color: "var(--color-text-secondary)" };
  const prio = FEEDBACK_PRIORITY_META[r.priority as keyof typeof FEEDBACK_PRIORITY_META] ?? { bg: "var(--color-surface-muted)", color: "var(--color-text-secondary)" };
