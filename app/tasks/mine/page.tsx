    ...
    <div className="max-w-[1000px] p-12 pt-9">
      ...
      <div className="mt-6 overflow-hidden rounded-card border border-[var(--color-border)] bg-[var(--color-surface)]">
        {TASKS_MINE.map((t) => (
          <div
            key={t.title}
            className="flex items-center gap-3.5 border-b border-[var(--color-ink-100)] px-5 py-3.5 text-sm last:border-b-0"
          >
            <span
            ...
            <span
              className="w-24 rounded-pill px-2.5 py-1 text-center text-[11px] font-semibold"
              style={{ background: t.stBg, color: t.stColor }}
            >
              {t.stL}
            </span>
            <span className="w-[78px] text-right text-xs text-[var(--color-text-secondary)]">{t.due}</span>
          </div>
        ))}
      </div>
    </div>
