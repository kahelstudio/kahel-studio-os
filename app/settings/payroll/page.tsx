  ...
  return (
    <div className="max-w-[820px] p-12 pt-9">
      ...
      <p className="mt-1 text-[15px] text-[var(--color-text-secondary)]">
        Schedule, calculation rules, government tables and workspace controls
      </p>
      ...
      <div className="mt-6 grid grid-cols-2 gap-5">
        {PAYROLL_SETTINGS_GROUPS.map((g) => (
          <div key={g.title} className="overflow-hidden rounded-card border border-[var(--color-border)] bg-[var(--color-surface)]">
            ...
            {g.items.map((item) => (
              <div key={item.l} className="flex items-center justify-between border-b border-[var(--color-border)] px-5 py-3 text-sm last:border-b-0">
