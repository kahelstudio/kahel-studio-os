...
export default async function TransactionalPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <MessagesWorkspace result={result} filters={filters} failedView={false} canRetry={["admin", "super_admin"].includes(principal.role)} canTest={principal.role === "super_admin"} />;
}
