...
function Activity({ label, when, color }: { label: string; when: string; color: string }) { return <div className="flex gap-3"><span className="mt-1.5 h-2 w-2 shrink-0 rounded-full" style={{ background: color }} /><div><div className="text-sm font-medium">{label}</div><div className="mt-0.5 text-xs text-[var(--color-text-secondary)]">{when}</div></div></div>; }
