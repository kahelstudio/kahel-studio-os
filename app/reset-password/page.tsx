...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      if (!token) setError("This password reset link is invalid or has expired.");
      else {
  ...
  async function resetPassword(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/staff/password-reset", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ accessToken, password }) });
    if (!response.ok) {
