...
export function ApprovalsClient({ initialDashboard, initialError }: { initialDashboard: ApprovalDashboard | null; initialError: string | null }) {
  ...
  return <div className="app-page min-w-0 p-4 pb-16 sm:p-7 lg:p-9">
    ...
    <header className="flex flex-col gap-5 border-b border-[var(--color-border)] bg-[var(--color-surface)] pb-9 lg:flex-row lg:items-end lg:justify-between">
      <div><h1 className="font-display text-[clamp(1.8rem,4vw,2.25rem)] font-semibold leading-11 tracking-[-0.025em]">Approvals</h1><p className="mt-1 text-[15px] text-[var(--color-text-secondary)]">Review and manage operational and financial requests.</p></div>
    ...
    <div className="mt-6 flex items-end gap-6 overflow-x-auto" role="tablist" aria-label="Approval views">
      {(["my-approvals", "my-requests", ...(dashboard.role !== "staff" ? ["all"] : []), "completed"] as Tab[]).map((item) => <button key={item} role="tab" aria-selected={tab === item} onClick={() => updateParams({ tab: item === "my-approvals" ? null : item, status: null, view: null })} className={`shrink-0 pb-3 text-sm font-semibold capitalize transition-colors ${tab === item ? "text-[#FF5300] underline decoration-[#FF5300] decoration-4 underline-offset-[6px]" : "text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)]"}`}>{tabLabel(item)}</button>)}
    </div>
    <div className="mt-3 flex items-end gap-6 overflow-x-auto" aria-label="Saved views">
      {secondaryViews.map((view) => <button key={view} onClick={() => updateParams({ view: searchParams.get("view") === view ? null : view, status: null })} className={`shrink-0 pb-3 text-sm font-semibold capitalize transition-colors ${searchParams.get("view") === view ? "text-[#FF5300] underline decoration-[#FF5300] decoration-4 underline-offset-[6px]" : "text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)]"}`}>{view.replace("-", " ")}</button>)}
