...
export async function GET() {
  try {
    const [bookings, pipeline, employees, expenses] = await Promise.all([
      ...
      getProjectPipeline(),
      getPayrollEmployees(),
      getSupabaseAdmin().from("expenses").select("total_amount_centavos,status,owner_funded,duplicate_of").in("status", ["approved", "scheduled_for_payment", "paid"]).is("duplicate_of", null),
    ]);
    ...
    const approvedExpenses = (expenses.data ?? []).reduce((sum, expense) => sum + expense.total_amount_centavos, 0);
    const ownerFundedCosts = (expenses.data ?? []).filter((expense) => expense.owner_funded).reduce((sum, expense) => sum + expense.total_amount_centavos, 0);
