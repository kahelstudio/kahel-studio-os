...
export async function POST(request: Request, { params }: Context) {
  ...
  const original = await admin.from("transactional_messages" as never).select("contains_secure_content").eq("id", messageId).maybeSingle<{ contains_secure_content: boolean }>();
  ...
  const audit = await admin.from("staff_audit_log").insert({ actor_id: principal.userId, actor_name: principal.email, event: "Transactional email manually requeued", event_type: "email", entity_type: "transactional_message", entity_id: result.data.id, metadata: { parent_message_id: messageId, reason, idempotency_key: idempotencyKey } });
  if (audit.error) console.error("[transactional-email] Manual resend audit failed", result.data.id, audit.error.code);
  return Response.json({ messageId: result.data.id, status: result.data.status, reused: false }, { status: 202 });
}
